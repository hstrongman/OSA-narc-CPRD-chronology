capture log close
log using "$logdir\31.cr_aggregated_prevalence_data.txt", replace text

/*******************************************************************************
# Stata do file:    31.cr_aggregated_prevalence_data.do
#
# Author:      Helen Strongman
#
# Date:        15/06/2023
#
# Description: 	Create aggregated data for prevalence analysis (stratified by
#				age, sex, country, and calendar time)
#				
...
foreach medcondition in OSA narcolepsy {
	...
	*** create locals describing the start and end year for the dataset
	...
	collapse (sum) `studypopyears' `prevcasesyears', by(yob gender country)
	
	/*** reshape to dataset with one row for each year and distinct strata grouping ***/
	reshape long studypop prevcases, i(yob gender country) j(year)
	...
	*replace agecat = 85 if agecat == . & _age >=85 & _age !=.
	...
	drop _age
	collapse (sum) studypop prevcases, by(year agecat gender country)
	pause
	...
	if `i' > 1 append using "$datadir_an/31.cr_aggregated_prevalence_data.dta"
	save "$datadir_an/31.cr_aggregated_prevalence_data.dta", replace
