capture log close
log using "$logdir\51.an_inc-prev_ratios_forestplot_calendartime.txt", replace text

/*******************************************************************************
# Stata do file:    51.an_inc-prev_ratios_forestplot_calendartime.do
#
...
#				incidence rate ratios comparing calendar years
#
# Requirements: 
#				
...
foreach medcondition in OSA narcolepsy {
	...
	local estshort "IRR"
	keep if covar == "calendaryear" & medcondition == "`medcondition'"

	
		/*added for this do file*/
	replace covarlabel = "Calendar year"
	destring valuelabel, replace
	replace valuelabel = value
	...
	*need crude and adjusted estimates to be separate rows
	...
	keep covar covarlabel value valuelabel `est'ratio `est'ratio_lci `est'ratio_uci `est'ratio_str `est'ratio_pstr model
	...
	*AND change varlist to varlist`graph' in foreach command
	...
	foreach covar of local varlist {
		...
		*gen plabpos = 12 /*location of p-value*/
		gen estlabpos = 60.0 /*location of estimates*/
		gen covarlabpos = 0.2  /*location of variable labels*/
		gen valuelabpos = 0.2 /*local of value labels*/
		...
		*******************************************************************************/
		...
		/// graph options
				...
				xlab(0.5 1 2 4 6, labsize(vsmall)) /// x-axis tick marks
...
grc1leg OSA_inc narcolepsy_inc, ///
legendfrom(OSA_inc) position(6) ///
graphregion(color(white)) ///
name(country, replace)
pause
graph display country, ysize(8) margins(tiny)
graph export "$resultdir/51.an_inc-prev_ratios_forestplot_calendaryear", as(emf) replace
